...
> - 안 맞는 기록 하나가 다른 기록들의 동기화까지 막던 문제 수정 — 이 두 가지로 오프라인 저장 안전장치(SYNC) 항목 7개 중 6개 완료, 남은 1개(저장 실패 시 알림 방식)는 화면 디자인이 나온 뒤 재검토
...
**축 요약** — 아래 WBS 표의 축 열을 합산한 값이라 따로 관리하지 않습니다: 유실방지 82% · 공격방어 72% · 약속이행 50% · 경험설계 52% · 기반구축 96%.
...
| 2.4 | `SYNC`(오프라인 동기화 큐) | 유실방지 | 86% | — | 5.1 | [부록 C](#sync--오프라인-동기화--저장-안전장치-2026-05-16--2026-07-11-2026-07-22-재점검-2026-07-24-2건-처리)(7개) |
| **3** | **서버 인프라 구축** | | | | | |
...
| 날씨 스냅샷 (OWM + Kakao) | ✅ 완료 | |
| **`SYNC` — 오프라인 동기화 + 저장 안전장치** | 🟡 진행중 (6/7) | 핵심 4건(`synced` 필드, `syncPendingRecords()`, 서버 `clientMutationId` 멱등 upsert 등)은 2026-07-11 완료. **2026-07-22 재점검**에서 신규 발견 3건 중 다중 탭 경합·영구 실패 기록 큐 차단 2건은 **2026-07-24 처리 완료**. 남은 1건(localStorage 쓰기 실패 무통보)은 UX와 연결돼 화면설계 후 재검토하기로 보류 |
...
| `SYNC-SAFETY` | **오프라인 저장 안전장치 보강** (2026-07-22 재점검 신규, 2026-07-24 1/2 처리) | `src/app/lib/recordsStore.ts` | ~~다중 탭 동시 저장 경합 방지~~(✓ 2026-07-24), localStorage 쓰기 실패(quota 초과·프라이빗 모드) 시 사용자 안내 — 화면설계 후 재검토 예정 | 저장이 조용히 실패하면 사용자는 "분명히 저장했는데 새로고침하니 기록이 사라졌다"를 겪게 됩니다 — 이 앱이 가장 피해야 할 경험입니다. `SYNC` 마일스톤이 2026-05~07에 완료 처리됐다가 이번 재점검에서 다시 열림(6/7) |
...
> `SYNC`(오프라인 동기화 큐) — 핵심 4건은 2026-07-11에 완료됐으나, **2026-07-22 재점검에서 다시 열림**. 신규 발견 3건 중 2건은 **2026-07-24에 처리**(다중 탭 경합·영구 실패 기록 큐 차단), 남은 1건(localStorage 쓰기 실패 무통보)은 위 High 표의 `SYNC-SAFETY` 참고 — 현재 6/7. 코드 레벨 상세는 [부록 A](#부록-a--저장-흐름-병목-auth-근거), 실행용 체크리스트는 [부록 C](#sync--오프라인-동기화--저장-안전장치-2026-05-16--2026-07-11-2026-07-22-재점검-2026-07-24-2건-처리) 참고.
...
설계 완료 대기 중인 항목(`PROFILE-UI`, `EDITOR-UI`, `EMPTY-UI` 대부분, `KEY-FEATURES`의 입력·감정태그 개편)을 제외하고, **지금 바로 착수 가능한 것부터 순서대로 처리 완료(2026-07-24)**: `SYNC` 세이프티 2건(다중 탭 경합·영구 실패 기록 큐 차단) → `ACCESSIBILITY` 5건(포커스 트랩·aria-label 전수 점검·포커스 링·키보드 완주·감사 중 발견한 ListItem aria-hidden 결함) → `DATA-INTEGRITY` 1건(데이터 내보내기 지원 스크립트). 다음으로 코드만으로 가능한 것:

1. `SYNC-SAFETY` 나머지 1건(localStorage 쓰기 실패 무통보) — UX 카피·화면(토스트? 배너?) 결정 필요, 디자인 대기
2. `AUTH` 남은 1건(토큰 만료 시 로그인 화면 유도) — UX 카피·화면 결정 필요, 디자인 대기
3. `SECURITY-HARDENING` § 배포 전 체크리스트 5건 — 실 배포 환경 필요, 로컬 검증 불가
4. `SYNC-LIVE` 남은 2건(로그인 유도 시점, 온보딩 업로드 흐름) — 사용자가 "UI 설계하면서 결정"하기로 함(2026-07-22)
5. `DATA-INTEGRITY` 나머지 4건 — 대부분 삭제 UI가 먼저 있어야 검증 가능(`EDITOR-UI`/`PROFILE-UI` 설계 대기)
...
### SYNC — 오프라인 동기화 + 저장 안전장치 (2026-05-16 ~ 2026-07-11, 2026-07-22 재점검, 2026-07-24 2건 처리)
...
| 예외처리 | **localStorage 쓰기 실패(quota 초과, Safari 프라이빗 모드 등) 시 사용자에게 전혀 알리지 않음** — 메모리 캐시엔 남아있어 화면엔 저장된 것처럼 보이지만, 새로고침하면 그 기록이 사라짐(`recordsStore.ts`의 `writeRaw()`가 실패를 조용히 삼킴). **[2026-07-24]** 사용자 판단: "디자인 의존도가 있어 보인다 — 실패를 어떻게 알릴지(토스트? 인라인 배너?)는 UX와도 연결되니 화면설계 후 다시 우선순위를 잡겠다" — 코드 착수 보류, WBS 2.4의 `최우선순위` 태그도 제거(남은 게 이 한 건뿐이고 지금은 디자인 대기 상태). **→ PM 참고:** "저장 안 됨"을 사용자가 알아야 다시 시도하거나 다른 방법을 찾을 기회가 생깁니다 — 지금은 그 신호 자체가 없습니다 | 상 | ☐ 대기(디자인 필요) |
| 예외처리 | 여러 탭에서 거의 동시에 저장하면 나중에 쓰는 탭이 먼저 탭의 새 기록을 덮어쓸 수 있음 — `writeRaw()`가 배열 전체를 덮어쓰는 구조라 병합이 아님 | 중 | ☑ 완료 — **[2026-07-24]** `saveRecord()`/`markRecordSynced()`가 각 탭의 메모리 캐시가 아니라 `readRaw()`로 락 안에서 최신 localStorage를 직접 읽도록 수정 + `remindApi.ts`의 refresh token 탭 경합과 같은 해법(Web Locks API)으로 읽기→병합→쓰기 구간을 탭 간에 직렬화. 브라우저에서 직접 재현해 검증: 한 탭의 메모리 캐시가 낡은 상태(다른 탭이 방금 쓴 기록 C를 모르는 상태)에서 저장을 실행해도 기존 C가 사라지지 않고 A·B·C 세 기록 모두 보존됨을 확인(수정 전 코드였다면 C가 덮어써져 사라졌을 시나리오) |
| 예외처리 | **특정 기록이 영구적으로 실패하면(예: 서버가 항상 거부하는 손상된 데이터) 그보다 오래된 미동기화 기록 전체가 함께 무기한 막힘** — 실패 건을 격리(skip)하고 나머지는 계속 진행하는 구조가 필요. 2026-07-22 SYNC-LIVE 통합 테스트에서 실제로 재현·확인된 한계(데이터 유실은 아님, 지연만 발생). **[2026-07-24]** `postQuickEntry()`가 던지는 에러에 HTTP 상태 코드를 보존하는 `SyncHttpError`를 신설 — 이전엔 상태 코드가 에러 메시지 문자열에만 섞여 있어 "이 기록은 영구히 무효(400·413)"와 "지금은 네트워크·서버 문제(5xx·오프라인·401)"를 구분할 방법이 없었음. `syncPendingRecords()`가 이제 400·413(zod 검증 실패·bodyLimit 초과 — 재시도해도 절대 안 바뀜)만 `continue`로 건너뛰고, 그 외(401·5xx·네트워크 다운)는 기존대로 즉시 중단하도록 분기. **→ PM 참고:** 지금 구조로는 안 고쳐지는 기록 하나가 그보다 오래된 모든 기록의 동기화를 계속 미룹니다 | 중 | ☑ 완료 |
