...
> 마지막 갱신: 2026-07-22c: **SYNC-LIVE, 설계 화면 없이 가능한 4건 처리(2/10 → 6/10, 60%)** — 사용자 질문: "유실방지 축 중 5.1이 왜 이렇게 낮은가, 설계 화면 없이도 가능한 태스크가 있으면 우선순위 잡아달라. 오늘 50% 넘기는 걸 목표로." 남은 8개 항목 중 설계·정책 결정이 필요 없는 4개 처리: ① 비로그인 로컬 전용 모드 유지 여부 — "일단 항상 유지"로 결정(로그인 없이 휴대폰에만 저장하고 싶은 사용자 고려, 코드 변경 없음). ② `GET /v1/entries` 목록 조회 엔드포인트 신규 구현(`server/src/app.ts`) — 나머지 항목 대부분의 전제조건이던 라우트. ③ 대량 업로드 `clientMutationId` 멱등성 재사용 확인(테스트). ④ 대량 업로드 부분 실패 시 재시도 큐 유지 확인(코드 리뷰 — `syncPendingRecords()`가 이미 만족하는 구조임을 확인, 프론트 테스트 하네스 없어 자동화는 별도 결정 필요). `server/src/app.test.ts` 테스트 8건 추가(총 31개 통과). SYNC-LIVE 2/10 → 6/10, WBS 5.1 완성도 20%→60%, 유실방지 축 67%→80%, 전체 완료 항목 74→78로 연쇄 갱신. 아티팩트에도 동일하게 반영
> 2026-07-22b: **"이전 버전으로 방치된 채 완료로 표시된" 항목 점검 + AUTH 하위호환 테스트 실제 검증(12/13)** — 사용자 질문: "AUTH는 UX만 남은 것 같고 예외처리는 배포 시점이라 거의 다 된 것 같은데, 로그인 UI가 없어서 더 체크할 게 없는 상황인가?" 실제 코드(`src/app/lib/remindApi.ts`)를 추적한 결과 `syncPendingRecords()`는 로그인 UI와 무관하게(메모리 `accessToken`만 있으면) 검증 가능함을 확인 — "배포 시점에"라는 기존 문구가 틀렸던 것. `server/src/app.test.ts`에 실제 검증 테스트 2건 추가(총 26개 통과)해 완료 처리, AUTH 11/13 → 12/13, WBS 4.1 완성도 85%→92%, 기반구축 축 92%→96%로 연쇄 갱신. 이어서 체크리스트 전체에서 같은 유형(2026-07-15 AUTH JWT 전환 이후 대체 사실이 반영 안 된 자리)을 점검해 5건 정정: ① `SECURITY-BASELINE`의 완료 행 2건이 지금은 없는 `ALLOW_DEV_AUTH`/`resolveDevEmail()`을 현재형으로 설명하던 것에 각주 추가(원문은 유지, 삭제하지 않음), ② 자동화 커버리지 현황의 "401 재시도 큐 분기"가 실제로는 2026-07-15에 구현 완료됐는데 여전히 "기획(AUTH 선행 필요)"로 분류돼 있던 것을 구현 완료로 재분류(기획 8→7, 구현완료 1→2). 공통점: 전부 "기능이 안 됐다"가 아니라 "설명이 코드를 못 따라갔다"는 유형
> 2026-07-21g: **배포 보안 체크 워크플로우에 남은 3건(CORS_ORIGIN·JWT_SECRET·HSTS)의 구체적 확인 방법 명시** — "나머지 3건도 배포 후 검증 워크플로우에 반영해줘"라는 요청에 따라, 기존 [배포 보안 체크 워크플로우](#배포-보안-체크-워크플로우) 1~3단계의 뭉뚱그린 서술을 실제 `curl` 명령과 확인 대상으로 구체화: ① `JWT_SECRET` 미기동 시 뜨는 정확한 FATAL 로그 문자열과 `curl -I .../health` 200 확인, `CORS_ORIGIN`은 위조 Origin 헤더로 호출해 반사되지 않는지 확인하는 명령 추가. ② 통신 보안 확인의 `curl -I` 문구를 실제 호출 형태로 구체화. ③ 로그 확인에 직전 항목에서 추가한 `LOG_LEVEL`·`REQUEST_LOG_REDACT`를 파일 경로까지 명시. 아티팩트에도 동일하게 반영(같은 섹션에서 폐기된 `ALLOW_DEV_AUTH` 크래시 가드 언급도 함께 `JWT_SECRET`으로 정정)
> 2026-07-21f: **WBS 4.2 배포 전 체크리스트 5건 중 2건을 미리 코드로 처리(로그 레벨·민감정보 마스킹)** — "배포환경이 필요하다는 항목들 중 미리 할 수 있는 게 뭔지 제시해달라"는 요청에 서버 코드(`server/src/app.ts`)를 직접 확인, 5건 중 3건(CORS_ORIGIN 안전 기본값·JWT_SECRET 가드·HSTS)은 이미 끝났거나 진짜 배포 전용이었고 나머지 2건은 지금 처리 가능해 바로 구현: ① `logger: true`(pino 기본 레벨 암묵 의존) → `LOG_LEVEL` 환경변수로 명시 제어. ② **[중요 정정]** 최초엔 "Authorization·Cookie 헤더가 매 요청 로그에 평문으로 남는다"고 판단했으나, Fastify 5의 실제 기본 serializer(`server/node_modules/fastify/lib/logger-pino.js`)를 직접 확인한 결과 method/url/statusCode만 남기고 헤더 자체를 로깅하지 않아 그 판단이 틀렸음이 테스트 작성 중 드러남 — 실제로는 새는 값이 없었음. 다만 향후 커스텀 로깅·serializer 교체로 새게 될 경우에 대비해 `REQUEST_LOG_REDACT`(Authorization/Cookie/Set-Cookie 마스킹) 안전장치는 그대로 추가하고, 유닛 테스트로 이 설정이 실제로 마스킹되는지까지 직접 검증(`server/src/app.test.ts`, 총 24개 테스트 통과, `tsc --noEmit` 클린). 아티팩트에도 동일하게 반영
> 2026-07-21e: **WBS 6.1 항목명을 `SECURITY-HARDENING § 배포 전 체크리스트`로 변경(md는 이 표기만 동기화)** — "WBS 4.2의 상세 링크를 눌렀는데 아코디언이 안 열려서 뭐가 있는지 모르겠다, 상세 섹션하고 WBS 이름이 안 맞는 것 같다"는 지적에 따라, 아티팩트에서 앵커 id 위치를 옮겨 아코디언 자동 펼침 버그를 수정(순수 HTML 표준 동작, md는 애초에 아코디언이 없어 해당 없음)하고, WBS 6.1이 "배포 전 체크리스트"라는 이름만으로는 어느 마일스톤 소속인지 안 보이던 걸 `SECURITY-HARDENING § 배포 전 체크리스트`로 바꿔 표 행 이름과 실제 상세 섹션 소속을 일치시킴 — md도 동일하게 반영(md의 "상세" 열은 원래부터 부록 하위 섹션 앵커라 실제 이동 대상은 그대로, 표기만 통일)
> 2026-07-21d: **WBS 섹션 상단부 군더더기 제거(아티팩트 반영, md는 태그명만 동기화)** — "'모든 게 여기서 시작합니다' 우산 문구는 우리가 다 아는 사실이라 숨겨도 되지 않을까, 그 아래 안내 문단도 md엔 있어도 되지만 아티팩트엔 안 보여도 될 것 같다. 선정 기준 태그도 부가설명 없이 레이블 자체를 쉬운 말로 바꾸면 된다"는 지적에 따라, 아티팩트의 `#service-policy` 섹션에서만 우산 라벨과 안내 문단을 삭제하고 `.wbs-legend`(태그별 뜻 설명) 블록도 삭제 — 이 두 가지는 마크다운 문서 성격상 계속 필요해 md에서는 유지. 다만 뜻풀이가 필요했던 태그 2개는 더 쉬운 말로 리네임해 아티팩트·md 동일하게 반영: `게이팅` → `선행조건`, `5기준` → `최우선순위`(나머지 6개 태그는 원래도 단어 자체가 뜻을 담고 있어 유지)
...
**축 요약** — 아래 WBS 표의 축 열을 합산한 값이라 따로 관리하지 않습니다: 유실방지 80% · 공격방어 72% · 약속이행 40% · 경험설계 36% · 기반구축 96%.
...
| **5** | **서버 동기화 연결** | | | | | |
| 5.1 | `SYNC-LIVE` | 유실방지 | 60% | `선행조건` `과잉설계방지` | 4.1, 2.4 | [부록 C](#sync-live--서버-동기화-활성화-실사용자-연결)(10개) |
| **6** | **정식 출시** | | | | | |
...
| **`SYNC-LIVE` — 서버 동기화 (실 사용자 연결)** | 🟡 진행중 (6/10) | 로그아웃/재로그인 시 로컬 데이터 무효화(`session.ts`, 2026-07-13) + `X-Dev-Email` → `Authorization: Bearer` 교체(`remindApi.ts`, 2026-07-15에 완료됐으나 이 줄만 뒤늦게 반영). **`GET /v1/entries`(목록 조회)가 체크리스트에 없던 걸 2026-07-16에 신규 항목으로 추가, 2026-07-22에 구현 완료**. 같은 날 비로그인 로컬 전용 모드 유지 결정 + 대량 업로드 멱등성·부분 실패 재시도 큐 유지 2건 검증 완료. `updatedAt` 증분 동기화는 별도 항목으로 분리해 필요성이 확인될 때까지 보류(2026-07-18). 나머지 4개(온보딩 UX 2건, 병합 전략 정책 결정, `updatedAt` 보류분)는 화면·정책 결정 필요해 대기 |
...
| 기능 | **`GET /v1/entries` 목록 조회 엔드포인트 구현** (신규 항목, 2026-07-16 추가) — 인증된 사용자의 `JournalEntry` 목록을 `deleted:false`만 필터링해 반환. 지금까지 서버엔 생성(`POST .../quick`)·삭제(`DELETE .../:id`)만 있고 **조회 자체가 아예 없어서** 모아보기가 로컬 데이터만 볼 수 있는 근본 원인. 나머지 SYNC-LIVE 항목(온보딩 업로드, 병합 전략 등) 대부분이 이 라우트를 전제로 함 | 상 | ☑ 완료 (2026-07-22) — `server/src/app.ts`에 구현, `userId`+`deleted:false` 필터링·최신순 정렬. `server/src/app.test.ts` 테스트 3건 추가(인증 없이 401, 본인 소유·미삭제만 반환, 타인 기록 비노출) |
...
| 기능 | 비로그인 상태에서도 로컬 전용 모드를 유지할지 정책 결정. **→ PM 참고:** 로그인을 강제할지, 로그인 없이도 계속 쓸 수 있게 둘지는 순수 제품 결정입니다 — 이게 정해져야 온보딩·가입 유도 전략을 짤 수 있습니다 | 중 | ☑ 완료 (2026-07-22) — **결정: "일단은 항상 유지"** — 로그인하지 않고 휴대폰에만 저장하고 싶은 사용자도 있기 때문. 로그인 강제 전환 없음, 코드 변경 불필요(현재 graceful degradation 동작이 곧 정책). 온보딩·가입 유도는 "로그인 안 하면 못 씀"이 아니라 "로그인하면 이런 게 더 좋아짐" 방향으로 설계 |
...
| 예외처리 | 대량 업로드 중 일부 실패 시 전체 롤백 대신 실패 건만 재시도 큐에 남기기. **→ PM 참고:** 온보딩 업로드 중 일부가 실패해도 나머지 기록은 안전하게 저장된다는 뜻입니다 — "업로드 중 하나라도 실패하면 전부 날아간다"는 최악의 시나리오를 막는 항목입니다 | 상 | ☑ 완료 (2026-07-22, 코드 리뷰로 검증) — `syncPendingRecords()`(`remindApi.ts`)는 트랜잭션이 아니라 순차 반복이라 애초에 "롤백"할 대상이 없음. 성공한 기록은 `markRecordSynced()`가 그 즉시 `localStorage`에 동기적으로 반영되고(`recordsStore.ts`), 실패 시점에 `return`으로 멈춘 이후 항목들은 `synced:false`로 남아 다음 온라인 복귀 때 자연히 재시도됨 — 요구사항을 이미 만족. 프론트 테스트 하네스가 없어(server만 vitest) 자동 테스트 대신 소스 직접 확인으로 검증 |
