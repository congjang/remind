...
> 마지막 갱신: 2026-07-13 (디자인 제외 백엔드·데이터 안정화 3건: `SYNC-LIVE` 로그아웃/재로그인 로컬 데이터 무효화, `DATA-INTEGRITY` 소프트 삭제 엔드포인트 + 테스트, `AUTH` 데이터 마이그레이션 전략 초안 + PM 아티팩트 QA 시나리오 맵·자동화 우선순위 도입, 자동화 커버리지 현황 신설)
