...
| `HYDRATION-MISMATCH` | **React 하이드레이션 불일치** (`Minified React error #418`) | `src/app/page.tsx` 추정 | 서버·클라이언트 렌더링 결과가 일치해 콘솔 에러 없이 로드 | **2026-08-13** Vercel 실배포 콘솔에서 재확인(이전 세션에서도 동일 관찰) — 화면은 정상 작동하지만 매 로드마다 콘솔 에러가 남습니다. 랜덤 헤드라인 문구 등 서버·클라이언트 간 비결정적 렌더링이 원인으로 추정되나 아직 근본 원인 조사·수정은 안 함 |
