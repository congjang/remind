...
> 마지막 갱신: 2026-07-22e: **SYNC 신규 3건의 축(5개 축 기준) 재검토** — 사용자 요청: "WBS에도 반영해주고, 5개 축에 맞춰서 카테고리 분류해줘"(직전 항목에서 새로 추가한 3건이 SYNC의 기존 축(유실방지)에 그냥 묻어간 것 아닌지 명시적으로 재검토해달라는 취지). 3건 각각 축을 다시 판단: ① localStorage 쓰기 실패 무통보 → 유실방지(직접적 손실). ② 다중 탭 저장 경합 → 유실방지(다른 탭 기록을 덮어써 사라짐, 역시 직접적 손실). ③ 영구 실패 기록이 큐를 막음 → 당장 유실은 아니지만, 동기화가 무기한 지연되는 동안 로컬 한 곳에만 존재하는 기간이 길어져 기기 분실 등 다른 원인에 의한 유실 노출 창이 커짐 — 간접적이나 유실방지로 결론. 3건 전부 재확인 결과 WBS 2.4의 기존 축과 일치해 축 태그 자체는 변경 없음(재검토 근거만 명시적으로 기록). 아티팩트에도 동일하게 반영
...
**축 요약** — 아래 WBS 표의 축 열을 합산한 값이라 따로 관리하지 않습니다: 유실방지 73% · 공격방어 72% · 약속이행 40% · 경험설계 36% · 기반구축 96%.
