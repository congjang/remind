...
> 마지막 갱신: 2026-07-22f: **다중 기기 병합 전략 결정 — "충돌 없음"으로 완료(SYNC-LIVE 6/10 → 7/10)** — 사용자가 "최신 것만 승자"로 결정해달라고 요청했으나, 실제 코드 경로를 확인하는 과정에서 그 정책 자체가 성립하지 않는다는 걸 발견해 먼저 되짚음: `GET /v1/entries`가 아직 클라이언트 어디서도 소비되지 않고(`feed/page.tsx`는 여전히 로컬만 읽음), 더 근본적으로 `JournalEntry.body`가 append-only(수정 불가)라 "같은 기록을 두 기기가 다르게 수정"하는 시나리오 자체가 이 데이터 모델에선 불가능함 — 각 기록은 `clientMutationId`로 고유 식별되는 독립 엔트리라 두 기기의 기록은 그냥 나란히 생성될 뿐 "승자"를 가릴 대상이 없음. "최신 것만 승자(오래된 것 버림)"를 문자 그대로 구현했다면 유효한 일기 기록을 삭제하는 버그가 됐을 것 — 유실방지 축과 정면 충돌. 이 판단을 공유하자 사용자가 동의, **"병합 충돌은 구조적으로 발생하지 않으며 두 기기 기록 모두 독립 레코드로 보존된다"**로 정책을 확정하고 완료 처리(추가 코드 없음, 기존 `clientMutationId` 멱등 로직 그대로 사용). 아울러 이 항목이 원래 인용하던 `data-migration.md` §5 "열린 질문"이 사실은 dev-placeholder 계정의 기기별 재소유 문제를 다루는 것이지 이 시나리오와 무관하다는 것도 함께 정정. WBS 5.1 완성도 60%→70%, 유실방지 축 73%→76%로 연쇄 갱신. 아티팩트에도 동일하게 반영
> 2026-07-22e: **SYNC 신규 3건의 축(5개 축 기준) 재검토** — 사용자 요청: "WBS에도 반영해주고, 5개 축에 맞춰서 카테고리 분류해줘"(직전 항목에서 새로 추가한 3건이 SYNC의 기존 축(유실방지)에 그냥 묻어간 것 아닌지 명시적으로 재검토해달라는 취지). 3건 각각 축을 다시 판단: ① localStorage 쓰기 실패 무통보 → 유실방지(직접적 손실). ② 다중 탭 저장 경합 → 유실방지(다른 탭 기록을 덮어써 사라짐, 역시 직접적 손실). ③ 영구 실패 기록이 큐를 막음 → 당장 유실은 아니지만, 동기화가 무기한 지연되는 동안 로컬 한 곳에만 존재하는 기간이 길어져 기기 분실 등 다른 원인에 의한 유실 노출 창이 커짐 — 간접적이나 유실방지로 결론. 3건 전부 재확인 결과 WBS 2.4의 기존 축과 일치해 축 태그 자체는 변경 없음(재검토 근거만 명시적으로 기록). 아티팩트에도 동일하게 반영
> 2026-07-22d: **SYNC(오프라인 동기화 + 저장 안전장치) 재점검 — 완료(4/4)에서 진행중(4/7)으로 재분류** — 사용자 지적: "너무 초반(2026-05-16~07-11)에만 해두고 점검이 안 되어있는 것 같다, 해피패스와 엣지케이스가 이것밖에 없을 리가 없다." `recordsStore.ts`·`page.tsx` 저장 흐름을 직접 재검토해 실제 코드에 존재하지만 체크리스트에 없던 위험 3건 발견·추가: ① **localStorage 쓰기 실패(quota 초과·프라이빗 모드) 시 무통보** — 화면엔 저장된 것처럼 보이지만 새로고침하면 사라짐(최우선순위 후보). ② **여러 탭 동시 저장 시 레코드 유실 가능** — `writeRaw()`가 배열 전체를 덮어쓰는 구조라 병합 아님. ③ **영구 실패 기록이 그보다 오래된 미동기화 기록 전체를 무기한 막음** — 2026-07-22 SYNC-LIVE 통합 테스트에서 이미 확인된 한계를 이 마일스톤에도 교차 반영, 같은 김에 SYNC-LIVE 쪽 해당 항목 설명도 "실제 통합 테스트로 검증"으로 갱신. "과잉대응 아닌지 살펴보고 명확한 것만" 요청에 따라 2건은 검토 후 제외: 스키마 버전 미인식 시 전체 초기화(코드에 이미 의도된 트레이드오프로 명시돼 있고 현재 도달 불가능한 경로), 백그라운드 재시도 실패 침묵(데이터 유실이 아니라 UX 가시성 문제). 마일스톤을 부록 D(완료된 마일스톤)에서 부록 C(남은 마일스톤)로 이동(`SYNC-LIVE` 바로 앞), WBS 2.4 완성도 100%→57%, `최우선순위` 태그 추가, 유실방지 축 80%→73%, 전체 체크 항목 112→115, 끝낸 마일스톤 7→6·진행중 8→9로 연쇄 갱신. [진행 현황 목록](#진행-현황-목록-위험도-우선순위)·[현재 위치](#현재-위치-전체-프로세스-기준) 표에도 반영. 아티팩트에도 동일하게 반영
> 2026-07-22c: **SYNC-LIVE, 설계 화면 없이 가능한 4건 처리(2/10 → 6/10, 60%)** — 사용자 질문: "유실방지 축 중 5.1이 왜 이렇게 낮은가, 설계 화면 없이도 가능한 태스크가 있으면 우선순위 잡아달라. 오늘 50% 넘기는 걸 목표로." 남은 8개 항목 중 설계·정책 결정이 필요 없는 4개 처리: ① 비로그인 로컬 전용 모드 유지 여부 — "일단 항상 유지"로 결정(로그인 없이 휴대폰에만 저장하고 싶은 사용자 고려, 코드 변경 없음). ② `GET /v1/entries` 목록 조회 엔드포인트 신규 구현(`server/src/app.ts`) — 나머지 항목 대부분의 전제조건이던 라우트. ③ 대량 업로드 `clientMutationId` 멱등성 재사용 확인(테스트). ④ 대량 업로드 부분 실패 시 재시도 큐 유지 확인(코드 리뷰 — `syncPendingRecords()`가 이미 만족하는 구조임을 확인, 프론트 테스트 하네스 없어 자동화는 별도 결정 필요). `server/src/app.test.ts` 테스트 8건 추가(총 31개 통과). SYNC-LIVE 2/10 → 6/10, WBS 5.1 완성도 20%→60%, 유실방지 축 67%→80%, 전체 완료 항목 74→78로 연쇄 갱신. 아티팩트에도 동일하게 반영
...
**축 요약** — 아래 WBS 표의 축 열을 합산한 값이라 따로 관리하지 않습니다: 유실방지 76% · 공격방어 72% · 약속이행 40% · 경험설계 36% · 기반구축 96%.
...
| 2.4 | `SYNC`(오프라인 동기화 큐) | 유실방지 | 57% | `최우선순위` | 5.1 | [부록 C](#sync--오프라인-동기화--저장-안전장치-2026-05-16--2026-07-11-2026-07-22-재점검)(7개) |
| **3** | **서버 인프라 구축** | | | | | |
...
| 날씨 스냅샷 (OWM + Kakao) | ✅ 완료 | |
| **`SYNC` — 오프라인 동기화 + 저장 안전장치** | 🟡 진행중 (4/7) | 핵심 4건(`synced` 필드, `syncPendingRecords()`, 서버 `clientMutationId` 멱등 upsert 등)은 2026-07-11 완료. **2026-07-22 재점검**에서 localStorage 쓰기 실패 무통보·다중 탭 경합·영구 실패 기록의 큐 차단 3건 신규 발견, 대기 |
...
| `SYNC-SAFETY` | **오프라인 저장 안전장치 보강** (2026-07-22 재점검 신규) | `src/app/lib/recordsStore.ts` | localStorage 쓰기 실패(quota 초과·프라이빗 모드) 시 사용자 안내, 다중 탭 동시 저장 경합 방지 | 저장이 조용히 실패하면 사용자는 "분명히 저장했는데 새로고침하니 기록이 사라졌다"를 겪게 됩니다 — 이 앱이 가장 피해야 할 경험입니다. `SYNC` 마일스톤이 2026-05~07에 완료 처리됐다가 이번 재점검에서 다시 열림(4/7) |
...
> `SYNC`(오프라인 동기화 큐) — 핵심 4건은 2026-07-11에 완료됐으나, **2026-07-22 재점검에서 다시 열림(4/7)** — localStorage 쓰기 실패 무통보 등 신규 발견 3건은 위 High 표의 `SYNC-SAFETY` 참고. 코드 레벨 상세는 [부록 A](#부록-a--저장-흐름-병목-auth-근거), 실행용 체크리스트는 [부록 C](#sync--오프라인-동기화--저장-안전장치-2026-05-16--2026-07-11-2026-07-22-재점검) 참고.
...
### SYNC — 오프라인 동기화 + 저장 안전장치 (2026-05-16 ~ 2026-07-11, 2026-07-22 재점검)

> 코드 레벨 상세는 [부록 A](#부록-a--저장-흐름-병목-auth-근거) 참고. **2026-07-22 재점검**: `recordsStore.ts`·`page.tsx` 저장 흐름을 직접 재검토해 실제 코드에 존재하지만 체크리스트에 없던 위험 3건 발견·추가(전부 코드에 실제로 존재하는 경로, 가정이 아님). 검토했지만 추가하지 않은 것 — 스키마 버전을 못 알아볼 때 전체 초기화(코드 주석에 이미 "데이터 손실 방지보다 무결성 우선"이라는 의도된 트레이드오프로 명시돼 있고, 현재 스키마 버전(1→2)만으로는 도달 불가능한 경로), 백그라운드 재시도 실패가 조용함(데이터 유실·손상이 아니라 UX 가시성 문제) — 둘 다 과잉대응으로 판단해 제외. **축 재검토(신규 3건, 5개 축 기준)**: ① localStorage 쓰기 실패 무통보 → **유실방지**(저장 자체가 안 되는 직접적 손실). ② 다중 탭 저장 경합 → **유실방지**(다른 탭의 기록을 덮어써 사라짐, 역시 직접적 손실). ③ 영구 실패 기록이 큐를 막음 → 당장 지워지진 않지만 동기화가 무기한 지연되는 동안 로컬 한 곳에만 존재하는 기간이 길어져 기기 분실·브라우저 데이터 삭제 등 다른 원인에 의한 유실 노출 창이 커짐 — 간접적이지만 결국 **유실방지** 축이 맞다고 판단, 기반구축·경험설계 등 다른 축으로 옮기지 않음. 3건 전부 WBS 2.4의 기존 축과 일치해 WBS 표 자체의 축 태그는 변경 없음.
...
| 데이터 | `syncPendingRecords()` 재시도(앱 시작·`online` 이벤트) + 서버 `clientMutationId` 멱등 upsert | 상 | ☑ 완료 — **[2026-07-22 참고]** 재시도 자체는 정상 동작하나, 아래 신규 항목("영구 실패 기록이 큐를 막음")에서 이 재시도 루프의 실제 한계가 별도로 추적됨 |
| 예외처리 | **localStorage 쓰기 실패(quota 초과, Safari 프라이빗 모드 등) 시 사용자에게 전혀 알리지 않음** — 메모리 캐시엔 남아있어 화면엔 저장된 것처럼 보이지만, 새로고침하면 그 기록이 사라짐(`recordsStore.ts`의 `writeRaw()`가 실패를 조용히 삼킴). **→ PM 참고:** "저장 안 됨"을 사용자가 알아야 다시 시도하거나 다른 방법을 찾을 기회가 생깁니다 — 지금은 그 신호 자체가 없습니다 | 상 | ☐ 대기 |
| 예외처리 | 여러 탭에서 거의 동시에 저장하면 나중에 쓰는 탭이 먼저 탭의 새 기록을 덮어쓸 수 있음 — `writeRaw()`가 배열 전체를 덮어쓰는 구조라 병합이 아님 | 중 | ☐ 대기 |
| 예외처리 | **특정 기록이 영구적으로 실패하면(예: 서버가 항상 거부하는 손상된 데이터) 그보다 오래된 미동기화 기록 전체가 함께 무기한 막힘** — 실패 건을 격리(skip)하고 나머지는 계속 진행하는 구조가 필요. 2026-07-22 SYNC-LIVE 통합 테스트에서 실제로 재현·확인된 한계(데이터 유실은 아님, 지연만 발생). **→ PM 참고:** 지금 구조로는 안 고쳐지는 기록 하나가 그보다 오래된 모든 기록의 동기화를 계속 미룹니다 | 중 | ☐ 대기 |
