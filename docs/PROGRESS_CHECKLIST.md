...
**5개 축 롤업** — 두 그룹으로 나눕니다. **지키는 축**(안 지키면 사용자를 배신하는 것)과 **만드는 축**(디자인씽킹의 Desirability·Feasibility 관점에서 제대로 만들고 있는가). **이 숫자는 아래 WBS 표의 축 열을 합산한 값**이라 따로 관리하지 않습니다:

| 그룹 | 축 | 완료 | 뜻 |
|---|---|---|---|
| 지키는 축 | 유실방지 | 20 / 30 (67%) | 사고로 데이터가 사라지지 않게 함(재시도, 멱등 처리, 오프라인 큐, 백업·복구) |
| 지키는 축 | 공격방어 | 13 / 18 (72%) | 외부 공격·오남용으로부터 보호(인증, 레이트 리미팅, 보안 헤더, 입력 검증) |
| 지키는 축 | 약속이행 | 4 / 10 (40%) | 사용자에게 한 약속을 지킴(데이터 권리, 법적 고지) — 이용약관 정식화(6.2)는 별도로 남아있음 |
| 만드는 축 | 경험설계 | 9 / 25 (36%) | 사용자가 쓰기 편한가(UX·디자인) |
| 만드는 축 | 기반구축 | 23 / 25 (92%) | 기술적으로 지탱 가능한가(인프라·게이팅) |
...
|---|---|---|---|---|---|---|
| **1** | **기획 · 설계** | | | | | |
| 1.1 | `PROJECT-SETUP` | 기반구축 | 100% | `게이팅` | — | [부록 D](#project-setup--프로젝트-초기-설정-2026-03-11--2026-03-19)(6개) |
| **2** | **로컬 프로토타입 (오프라인 우선)** | | | | | |
...
| **3** | **서버 인프라 구축** | | | | | |
| 3.1 | `SERVER-SCAFFOLD` | 기반구축 | 100% | `게이팅` | 4.1 | [부록 D](#server-scaffold--fastify--prisma-api-서버-골격)(6개) |
| 3.2 | `SECURITY-BASELINE` | 공격방어 | 100% | — | 4.2 | [부록 D](#security-baseline--최소-보안-가드-2026-04-17)(3개) |
| **4** | **📍 인증 · 보안 구축 — 현재 위치** | | | | | |
| 4.1 | `AUTH` | 기반구축 | 85% | `5기준` `게이팅` | 3.1, 4.2, 5.1, P.4 | [부록 C](#auth--신규-서버-실사용자-인증-jwt)(13개) |
| 4.2 | `SECURITY-HARDENING` | 공격방어 | 67% | `배포환경필요` | 4.1, 6.1 | [부록 C](#security-hardening--서버-보안-강화)(15개) |
| **5** | **서버 동기화 연결** | | | | | |
| 5.1 | `SYNC-LIVE` | 유실방지 | 20% | `게이팅` `과잉설계방지` | 4.1, 2.4 | [부록 C](#sync-live--서버-동기화-활성화-실사용자-연결)(10개) |
| **6** | **정식 출시** | | | | | |
