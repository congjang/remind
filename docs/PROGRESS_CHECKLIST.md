...
| 기능 | ~~카카오 역지오코딩(선택, 시·구·동 단위)~~ | 하 | **[2026-08-14] 제거** — 최초 커밋 때부터 코드에 있었지만 실제로 발급받아 쓴 API 키가 아니었음(사용자 확인). `KAKAO_REST_API_KEY` 없이도 원래 OWM 응답의 도시명으로 대체 표시되는 구조라 기능 손실은 없음, 관련 코드·문서(`route.ts`, `.env.example`, `CLAUDE.md`)에서 전부 제거 |
