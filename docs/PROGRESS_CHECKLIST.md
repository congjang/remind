...
> 태스크를 그때그때 확인·추가하다 보니 체크리스트가 방대해졌다는 지적에 따라 작업분류체계(WBS)로 정리했습니다(2026-07-18 신설, [PM 아티팩트](https://claude.ai/code/artifact/90835380-3597-45fb-9e97-36e796a9699d)에 동일 내용 있음 — 아티팩트가 시각적 원본, 여기는 표 요약). **2026-07-19/21 갱신**: 아티팩트에서 WBS를 중심 구조로 재편(Product Integrity 현황판을 PI축 열로 흡수, 관련·상세 열 추가, 마일스톤 상세를 아코디언으로 전환)한 것을 이 문서에도 동일하게 반영 — 마크다운은 아코디언·탭 같은 인터랙션은 표현할 수 없어 그 부분만 제외하고, **표 구조(PI축·관련·상세 열)와 Product Integrity 통합은 그대로 반영**했습니다.
...
**Product Integrity 롤업** — `SYNC`(기능 무결성)·`SECURITY`(리스크 방어)는 판단 기준이 다른 별개 축이지만 둘 다 "제품이 사용자를 배신하지 않는다"는 같은 상위 목표를 섬깁니다. **이 숫자는 아래 WBS 표의 PI축 열을 합산한 값**이라 따로 관리하지 않습니다:

| 축 | 완료 | 분류 기준 |
|---|---|---|
| 기능 무결성 (SYNC) | 6 / 14 (43%) | 사용자가 **데이터 유실 없이** 서비스를 이용하게 하는 핵심 로직(재시도, 멱등 처리, 오프라인 큐, 백업·복구) |
| 리스크 방어 (SECURITY) | 13 / 18 (72%) | **외부 위협이나 오남용**으로부터 서비스를 보호하는 방어적 로직(인증, 레이트 리미팅, 보안 헤더, 입력 검증) |
| **Product Integrity 전체** | **19 / 32 (59%)** | 새 작업이 위 기준에 명확히 맞으면 묻지 않고 해당 축으로 분류 — 애매하면 근거를 적고 승인받은 뒤 반영 |

`AUTH`·`DATA-INTEGRITY`는 아래 표에서 PI축이 "—"입니다 — 정의상 리스크 방어/기능 무결성에 걸치지만 각각 **게이팅 인프라**(거의 모든 후속 작업의 전제) / **데이터 소유권**(제3의 성격)이라는 별개 이유로 PI축 밖에 독립 항목으로 유지하기로 2026-07-11에 승인했습니다.

**선정 기준 범례** — "선정 기준" 열에 쓰인 태그의 뜻:

| 태그 | 뜻 |
|---|---|
| `게이팅` | 다른 작업 전부가 이걸 전제로 함 |
| `5기준` | 치명적·시급·최상중요도·빠른해결·비-디자인 5가지로 판단 |
...
| WBS | 작업 항목 | PI축 | 완성도 | 선정 기준 | 관련 | 상세 |
|---|---|---|---|---|---|---|
| **1** | **기획 · 설계** | | | | | |
| 1.1 | `PROJECT-SETUP` | — | 100% | `게이팅` | — | [부록 D](#project-setup--프로젝트-초기-설정-2026-03-11--2026-03-19)(6개) |
| **2** | **로컬 프로토타입 (오프라인 우선)** | | | | | |
| 2.1 | `KEY-FEATURES` | SYNC | 71% | — | P.1 | [부록 C](#key-features--마이크로-저널링-핵심-플로우)(7개) |
| 2.2 | `WEATHER-SNAPSHOT` | SYNC | 100% | — | — | [부록 D](#weather-snapshot--날씨위치-스냅샷-2025-03-24)(4개) |
| 2.3 | `TIMELINE-UI` | SYNC | 100% | — | P.2 | [부록 D](#timeline-ui--타임라인-피드-2026-04-01)(5개) |
| 2.4 | `SYNC`(오프라인 동기화 큐) | SYNC | 100% | — | 5.1 | [부록 D](#sync--오프라인-동기화--저장-안전장치-2026-05-16--2026-07-11)(4개) |
| **3** | **서버 인프라 구축** | | | | | |
| 3.1 | `SERVER-SCAFFOLD` | — | 100% | `게이팅` | 4.1 | [부록 D](#server-scaffold--fastify--prisma-api-서버-골격)(6개) |
| 3.2 | `SECURITY-BASELINE` | SECURITY | 100% | — | 4.2 | [부록 D](#security-baseline--최소-보안-가드-2026-04-17)(3개) |
| **4** | **📍 인증 · 보안 구축 — 현재 위치** | | | | | |
| 4.1 | `AUTH` | — | 85% | `5기준` `게이팅` | 3.1, 4.2, 5.1, P.4 | [부록 C](#auth--신규-서버-실사용자-인증-jwt)(13개) |
| 4.2 | `SECURITY-HARDENING` | SECURITY | 67% | `배포환경필요` | 4.1, 6.1 | [부록 C](#security-hardening--서버-보안-강화)(15개) |
| **5** | **서버 동기화 연결** | | | | | |
| 5.1 | `SYNC-LIVE` | SYNC | 20% | `게이팅` `과잉설계방지` | 4.1, 2.4 | [부록 C](#sync-live--서버-동기화-활성화-실사용자-연결)(10개) |
| **6** | **정식 출시** | | | | | |
| 6.1 | 배포 전 체크리스트 | SECURITY | 0% | `배포환경필요` | 4.2(하위) | [부록 C § 배포 전 체크리스트](#배포-전-체크리스트)(5개) |
| 6.2 | 이용약관 · 개인정보처리방침 정식화 | — | 0% | `정책선행` | P.4 | [이용약관](https://claude.ai/code/artifact/f73a98db-af7d-485f-a0ba-6dc0461e62d0) · [개인정보처리방침](https://claude.ai/code/artifact/fc87268b-7ee7-4e85-9c86-30a4c7e09c20) |
| **7** | **AI · 네이티브 확장** | | | | | |
| 7.1 | `AI-PIPELINE` | — | 0% | `후순위` | — | 체크리스트 미착수 |
| 7.2 | `PUSH-NOTIFICATION` · `NATIVE-APP` · `WIDGET` | — | 0% | `후순위` | — | 체크리스트 미착수 |
| **P** | **병행 트랙 — 순차 단계에 속하지 않는 UX·데이터 품질** | | | | | |
| P.1 | `DESIGN-SYSTEM` | — | 67% | `디자인선행필요` | 2.1 | [부록 C](#design-system--디자인-토큰아이콘storybook-파이프라인)(9개) |
| P.2 | `EMPTY-UI` | — | 0% | `후순위` | 2.3 | [부록 C](#empty-ui--피드-빈-상태-ux)(8개) |
| P.3 | `ACCESSIBILITY` | — | 38% | `저비용고임팩트` | — | [부록 C](#accessibility--접근성a11y-최소-세트)(8개) |
| P.4 | `DATA-INTEGRITY` | — | 40% | `저비용고임팩트` `과잉설계방지` | 4.1, 6.2 | [부록 C](#data-integrity--데이터-삭제무결성-보장)(10개) |
...
> 로그인 자체의 브루트포스 방어는 `AUTH` 체크리스트에 있고, 여기는 그 **바깥의 나머지 공개 엔드포인트**를 다룹니다. **2026-07-11부터 두 섹션으로 분리 관리**: 코드베이스에 항상 적용돼야 하는 [상시 보안 방어](#상시-보안-방어)와, 실제 배포 환경에서만 검증 가능한 [배포 전 체크리스트](#배포-전-체크리스트). 진행률은 두 섹션을 합산해 위 [WBS 표의 PI축(SECURITY)](#wbs--무엇을-왜-그-순서로-진행했나)에 반영됩니다.
